...
class PassGen : ListenerAdapter() {
    ...
    override fun onSlashCommandInteraction(event: SlashCommandInteractionEvent) {
        ...
        if (maxChar > 1000) {
            event.hook.sendMessage("❌ **Maximum character limit is 1000.**").queue()
            return
        }
        ...
        val embed = EmbedBuilder()
            ...
            **Type:** $characterType
        """.trimIndent(), false)
            .setFooter("🔒 Generated by ${event.user.name} on $timestamp", event.user.effectiveAvatarUrl)
            .setColor(Color.decode(api.getConfig("WORKERCOLOR")))
            .build()

        event.user.openPrivateChannel().queue({ channel ->
            channel.sendMessageEmbeds(embed).queue(
                { event.hook.sendMessage("✅ **Password sent to your private messages!**").queue() },
                { event.hook.sendMessage("❌ **Could not send a private message. Please check your settings.** Password: **```$password```**").queue() }
            )
        }, {
            event.hook.sendMessage("❌ **Could not open a private message channel. Your password is shown below:**\n**```$password```**").queue()
        })
    }
